...
def calcular_modelo(eps, ms, X, Y, log2_n_classes):
    try:
        modelo = DBSCAN(eps=eps, min_samples=ms, algorithm='kd_tree', n_jobs=1)
        ...
        if len(set(labels)) - (1 if -1 in labels else 0) > 1:
            ...
            return {
                ...
                "params": f"eps={eps:.3f}, ms={ms}"
            }
    except: pass
    return None

def main():
    print("Busca Exaustiva DBSCAN (8 Métricas)...")
    dados = ler_dados()
    X_raw = dados.iloc[:, :-1]
    Y = dados.iloc[:, -1]
    ...
    ms_range = range(5, 31, 2)
    parametros = [(eps, ms) for eps in eps_range for ms in ms_range]

    res_raw = Parallel(n_jobs=-1)(delayed(calcular_modelo)(eps, ms, X, Y, log2_n) for eps, ms in parametros)
