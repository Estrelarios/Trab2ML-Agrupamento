...
def calcular_modelo(n_clusters, linkage, metric, X, Y, log2_n_classes):
    try:
        if linkage == "ward" and metric != "euclidean":
            return None
            
        modelo = AgglomerativeClustering(n_clusters=n_clusters, linkage=linkage, metric=metric)
        ...
        if len(set(labels)) > 1:
            ...
            return {
                ...
                "params": f"n_clusters={n_clusters}, linkage={linkage}, metric={metric}"
            }
    except: pass
    return None

def main():
    print("Busca Exaustiva AGNES (8 Métricas)...")
    dados = ler_dados()
    X_raw = dados.iloc[:, :-1]
    Y = dados.iloc[:, -1]
    ...
    metric_range = ["euclidean", "manhattan", "cosine"]
    parametros = [(n, l, m) for n in n_clusters_range for l in linkage_range for m in metric_range]

    res_raw = Parallel(n_jobs=-1)(delayed(calcular_modelo)(n, l, m, X, Y, log2_n) for n, l, m in parametros)
